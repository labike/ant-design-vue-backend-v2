...
<template>
  <a-card :bordered="false">
    ...
    >
      ...
      <span slot="qq" slot-scope="index, text">
        <span>{{ text.qq }}</span>
      </span>
      ...
      <span slot="action" slot-scope="text, record">
        <template>
          ...
          <!-- <a @click="handleLoginLog(record)" :style="{display: 'inline-block', marginTop: '10px'}">登录日志</a> -->
          <a-divider type="vertical" />
          <a @click="handleSalaryDetail(record)" :style="{display: 'inline-block', marginTop: '10px'}">资金明细</a>
        </template>
      </span>
    </s-table>
...
import { shopperList, deleteShopper } from '@/api/user'
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return shopperList(Object.assign(parameter, this.queryParam))
          .then(res => {
            return res.data
          })
      },
